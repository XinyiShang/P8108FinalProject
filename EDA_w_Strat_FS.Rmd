...
forward_model = stepAIC(
  coxph(Surv(n_days, status) ~ ., data = cirrhosis |>
...
forward_model_interact = stepAIC(
  ...
  scope = list(lower = ~ 1, upper = as.formula(paste(
    ...
    paste0(
      "drug:",
      cirrhosis |> dplyr::select(-id, -n_days, -status, -drug) |> colnames(),
      collapse = " + "
    )
  ))),
  tract = F,
...
forward_model_interact = coxph(as.formula(
  paste(
    "Surv(n_days, status)  ~",
...
backward_model = coxph(as.formula(
  paste("Surv(n_days, status)  ~", backward_model$formula[3], "+ drug")
), data = cirrhosis |>
  dplyr::select(-id) |> na.omit())
...
stepwise_model = coxph(as.formula(
  paste("Surv(n_days, status)  ~", stepwise_model$formula[3], "+ drug")
), data = cirrhosis |>
  dplyr::select(-id) |> na.omit())
